...
# Checking for WordPress, Joomla, Drupal, phpBB, and Magento installs and 
# versions on a Media Temple Grid, DV's with Plesk or cPanel/WHM, and DV Developers

##################################################################

# Allow for exiting the script without dropping the SSH connection
function bye ()
  {
  return 0
  }
...
# If on Plesk or a "DV Developer"
  ...
  if [ "$(id -u)" != "0" ]; then
	echo "This needs to run as root or sudo. Exiting..."
	bye
  else
	echo "Looking for WordPress..."
